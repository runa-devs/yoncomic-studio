...
import { AlignVerticalJustifyStart, BookOpen, Settings2, Share2, Type } from "lucide-react";
...
const ImageEditor = React.forwardRef<ImageEditorRef, ImageEditorProps>(
  ({ imageUrl, width, height, comicId, story, onSave }, ref) => {
    ...
    const handleStageClick = (evt: KonvaEventObject<MouseEvent>) => {
      ...
      // クリックされた要素が吹き出しかたは四角形かどうかをチェック
      const clickedShape = evt.target;
      const isBubble = clickedShape.attrs?.id?.startsWith("bubble-");
      const isRect = clickedShape.attrs?.id?.startsWith("rect-");

      // 空白部分、吹き出し、または四角形をクリックした場合にテキストを追加
      if (evt.target === evt.target.getStage() || isBubble || isRect) {
    ...
    // 画像をFormDataとして取得するメソッド
    const getImageFormData = async () => {
    ...
    return (
      <div className="relative">
                  ...
                  <KonvaImage
                    ...
                    onClick={(e) => {
                      if (currentTool === "text") {
                        ...
                        handleNodeClick(shape.id, "shape");
                      }
                    }}
                    onTransform={handleShapeTransform}
                  />
        ...
        {currentTool === "hand" && selectedId && (
          <div className="fixed right-6 top-14 z-50">
            <Popover>
        ...
        <div className="fixed bottom-4 right-4 z-50">
          <Button
            ...
            className="size-10 bg-white/80 backdrop-blur-sm hover:bg-white"
            onClick={() => {
              router.push(`/comics/${comicId}/share`);
            }}
          >
        ...
        <Dialog open={showStoryDialog} onOpenChange={setShowStoryDialog}>
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              <DialogTitle>ストーリー</DialogTitle>
            </DialogHeader>
            <div className="mt-4 max-h-[60vh] overflow-y-auto">
              <div className="whitespace-pre-wrap text-sm">{story}</div>
            </div>
          </DialogContent>
        </Dialog>
      </div>
...
export interface ImageEditorRef {
  getImageFormData: () => Promise<FormData | null>;
}
